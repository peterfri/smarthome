...
class iAQ_Stick():
    def __init__(self, smarthome, update_cycle = "30"):
        self._sh = smarthome
    ...
    def xfer_type1(self, dev, msg):
        #dev.read(0x81, 0x10, self._intf, 1000)
        ...
        #logger.debug('iaqstick: write result {}'.format(ret))
    ...
    def _close(self):
        try:
            ...
            logger.error("iaqstick: releasing interfaces failed - {}".format(e))
    ...
    def _update_values(self):
        logger.debug("iaqstick: update sticks: {}".format(len(self._devs)))
        if len(self._devs) == 0 or len(self._items) != len(self._devs):
            logger.debug("devs: %d items: %d", len(self._devs), len(self._items))
            self._init()
        i = 0;
        for dev in self._devs:
            logger.debug("iaqstick: for {}".format(self._devs[dev]['id']))
            found_value = False;
            for k in range(0, 5):
                try:
                    self.read(dev)
